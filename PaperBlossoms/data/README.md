...
`paperblossoms.db` is a standard SQLite file, and can be inspected with any tool compatible with SQLite. The Paperblossoms team uses [DB Browser for SQLite](https://sqlitebrowser.org) and the [SQLite extension for VS Code](https://marketplace.visualstudio.com/items?itemName=alexcvzz.vscode-sqlite), but you can use any tool that works with SQLite.
...
In order to get the magical default snippet behaviour, VS Code must be [configured to map](https://code.visualstudio.com/docs/languages/json#_mapping-to-a-schema-in-the-workspace) a JSON file with its schema. Personally, I map using the `json.schemas` field in workspace settings. To improve portability, I provide `vscode_workspace_settings.json` which contain all the mappings, as well as a few settings I find useful. Your mileage may vary.
...
- All possible strings which could be translated are designated with `*_tr` fields in the database. Paperblossoms includes a utility to generate a spreadsheet with a list strings to be translated.
...
- At application launch, Paperblossoms automatically loads the appropriate CSV into the `i18n` table in the application database (based on the same logic ).
...
0. Set up the mapping between JSON files and their schemas (see [JSON schemas](#json-schemas)).
1. Add the new object(s) to the relevant JSON file.
    ...
    - Nested JSON objects come with nested snippets, trigger the snippet for a new object with <kbd>ctrl</kbd>-<kbd>space</kbd> once your cursor is within `{}`.
...
- Speaking of encoding in UTF-8, the Python function `json.load()` defaults to `encoding = 'utf8'` on *nix systems but not on Windows. I have not yet refactored `json.load()` to specify `encoding = 'utf8'` everywhere (only the ones I happened to work on while on Windows), so if you are on Windows and attempt run any scripts which call `json.load()` without encoding argument, you'll get an error. Simply fix the offending function call to specify encoding and try again. (Then include it in your PR!)

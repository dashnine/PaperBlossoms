...
NewCharWizardPage2::NewCharWizardPage2(DatabaseDependency* deps, QWidget *parent) :
    QWizardPage(parent),
    ui(new Ui::NewCharWizardPage2)
{
    ...
    QStringList kitsuneschoollist = deps->schoolsRepository->qsl_getschools(field("currentClan").toString(), true);
    ...
    if(field("characterType").toString() == "Samurai"){
        schoolModel->setStringList(deps->schoolsRepository->qsl_getschools(field("currentClan").toString()));
    }
    else if (field("characterType").toString() == "Gaijin") { //gaijin have ronin schools or their local schools
        schoolModel->setStringList(deps->schoolsRepository->qsl_getschools(deps->regionsRepository->qs_getregionsubtype(field("currentRegion").toString()), false, field("characterType").toString()) );
    }
    else{
        schoolModel->setStringList(deps->schoolsRepository->qsl_getschools("Rōnin", false, field("characterType").toString() )); //peasants and roning just have ronin schools
...
void NewCharWizardPage2::equipSelectionChanged(const QString newText){
    ...
    const QList<QStringList> equipsets = deps->schoolsRepository->ql_getlistsofeq(ui->nc2_school_ComboBox->currentText());              //get a list of equipsets
...
void NewCharWizardPage2::handleSpecCases(QString speccase){
    ...
    if(specCase == "One Weapon of Rarity 6 or Lower"){
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getweaponsunderrarity(6));
    }
        ...
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getitemsunderrarity(4));
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getitemsunderrarity(4));

    }
    else if (specCase == "One Sword of Rarity 7 or Lower") {
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getweapontypeunderrarity(7, "Swords"));
    ...
    //POW
        ...
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getweaponsunderrarity(8));
    }
    else if (specCase == "One Item of Rarity 3 or Lower") {
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getitemsunderrarity(3));
    }
    else if (specCase == "One Item of Rarity 4 or Lower") {
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getitemsunderrarity(4));
    }
    else if (specCase == "One Item of Rarity 5 or Lower") {
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getitemsunderrarity(5));
    }
    else if (specCase == "One Item of Rarity 6 or Lower") {
        ui->equipSpecialWidget->addCBox(deps->inventoryRepository->qsl_getitemsunderrarity(6));
    }
    //FoV
...
void NewCharWizardPage2::initializePage(){
    ...
    if(field("characterType").toString() == "Samurai"){
        ...
        ui->q3_groupBox->setTitle("3. What is your school, and what are its associated roles?");
        ui->q4_groupBox->setTitle("4. What gets you in and out of trouble?");
        schoolModel->setStringList(deps->schoolsRepository->qsl_getschools("Rōnin", false, "Rōnin" )); //peasants and roning just have ronin schools
...
bool NewCharWizardPage2::validatePage(){
    ...
    if(skillSelModel->rowCount() <  deps->schoolsRepository->i_getschoolskillcount(ui->nc2_school_ComboBox->currentText())){
        QMessageBox msg;
...
{
    schoolModel->setStringList(deps->schoolsRepository->qsl_getschools(field("currentClan").toString(), checked));

    if(field("characterType").toString() == "Samurai"){
        schoolModel->setStringList(deps->schoolsRepository->qsl_getschools(field("currentClan").toString(), checked));
    }
    else if (field("characterType").toString() == "Gaijin") { //gaijin have ronin schools or their local schools
        schoolModel->setStringList(deps->schoolsRepository->qsl_getschools(deps->regionsRepository->qs_getregionsubtype(field("currentRegion").toString()), checked, "Gaijin") );
    }
    else{
        schoolModel->setStringList(deps->schoolsRepository->qsl_getschools("Rōnin", checked, "Rōnin" )); //peasants and roning just have ronin schools
...
void NewCharWizardPage2::on_nc2_school_ComboBox_currentIndexChanged(const QString &arg1)
{
    ...
    //set the desc
    ui->nc2_schoolDesc_textEdit->setText(deps->schoolsRepository->qs_getschooldesc(arg1)+ " " + deps->schoolsRepository->qs_getschoolref(arg1));

    qDebug() << "School changed to:  " + arg1;
    //ui->nc2_schooldesc_textEdit->setText(deps->schoolsRepository->qs_getschooldesc(arg1));
    const int skillcount = deps->schoolsRepository->i_getschoolskillcount(arg1);
    qDebug()<< skillcount;
    const QString skilllabel = "Choose " + QString::number(skillcount) + " skills:";
    ui->nc2_skill_Label->setText(skilllabel);
    skillOptModel->setStringList(deps->schoolsRepository->qsl_getschoolskills(arg1)); //set list with school contents
    skillSelModel->setStringList( QStringList{} );  //clear prior selections, since this changed

    const QStringList subcategories = deps->techniquesRepository->qsl_gettechniquessubtypes();

    //TECHNIQUES//
    ui->techWidget->clear();
    const QList<QStringList> techsets = deps->schoolsRepository->ql_getlistsoftech(arg1);
            ...
            for (const auto& option : choicesetforcombobox){
                ...
                for (const auto& subcategory : subcategories){
                    ...
                    if (subcategory == option) {
                        map << deps->techniquesRepository->qsl_gettechniquesbysubcategory(option, 1, 1);
                        added = true;
                    }
                }
    ...
    //RINGS//
    ui->ringWidget->clear();
    const QStringList schoolrings = deps->schoolsRepository->qsl_getschoolrings(arg1);
    foreach(const QString ring, schoolrings){

        QStringList choicesetforcombobox;

        if(ring == "any"){
            choicesetforcombobox = deps->ringsRepository->qsl_getrings();
        }
        else{
...
void NewCharWizardPage2::doAddSkill(){
    ...
    if (sellist.count() >= deps->schoolsRepository->i_getschoolskillcount(ui->nc2_school_ComboBox->currentText())) return;

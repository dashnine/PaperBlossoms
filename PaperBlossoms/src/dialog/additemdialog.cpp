...
AddItemDialog::AddItemDialog(DatabaseDependency* deps, Character* character, QString type, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddItemDialog)
{
    ...
    this->adjustSize();
    ui->itemtemplate_combobox->addItems(deps->inventoryRepository->qsl_getitemsbytype(type));

    ui->qual_listView->setModel(&qualities);
    ui->qual_comboBox->addItems(deps->inventoryRepository->qsl_getqualities());
    ui->pattern_comboBox->addItems(deps->inventoryRepository->qsl_getpatterns());

    ui->grip1_category_comboBox->clear();
    ui->grip1_category_comboBox->addItems(deps->inventoryRepository->qsl_getweaponcategories());

    ui->grip2_category_comboBox->clear();
    ui->grip2_category_comboBox->addItems(deps->inventoryRepository->qsl_getweaponcategories());

    ui->grip1_skill_comboBox->clear();
    ui->grip1_skill_comboBox->addItems(deps->inventoryRepository->qsl_getweaponskills());

    ui->grip2_skill_comboBox->clear();
    ui->grip2_skill_comboBox->addItems(deps->inventoryRepository->qsl_getweaponskills());
...
void AddItemDialog::on_itemtemplate_combobox_currentIndexChanged(const QString &arg1)
{
    ...
    QStringList basedata = deps->inventoryRepository->qsl_getbaseitemdata(arg1, type);
    ...
    qualities.setStringList(deps->inventoryRepository->qsl_getitemqualities(arg1,type));
    //ui->qual_comboBox->addItems(deps->inventoryRepository->qsl_getqualities());

    if(type == "Weapon"){
        const QList<QStringList> weapondata = deps->inventoryRepository->ql_getweapondata(arg1);
        bool didFirst = false;
        ...
        const QList<QStringList> armordata = deps->inventoryRepository->ql_getarmordata(arg1);
